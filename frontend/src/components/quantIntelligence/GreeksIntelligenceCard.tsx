...
import {
  greeksIntelligenceApi,
  GreeksIntelligenceCard,
  UpdateGreeksIntelligenceRequest,
} from '../../api/client'
import { C, F } from '../../styles/tokens'

// ── Section definitions ────────────────────────────────────────────────────────

const SECTIONS: { key: keyof UpdateGreeksIntelligenceRequest; label: string; color: string }[] = [
  { key: 'whatItMeasures',       label: 'What it measures',             color: C.blue },
  { key: 'whyItMatters',         label: 'Why it matters in P&L',        color: C.blue },
  { key: 'commonMisuse',         label: 'Common misuse',                color: C.red },
  { key: 'positiveEvConditions', label: 'Positive expected value (EV)', color: C.green },
  { key: 'regimeContext',        label: 'Regime context',               color: C.amber },
  { key: 'sizingImplications',   label: 'Sizing implications',          color: C.textSub },
  { key: 'portfolioImpact',      label: 'Portfolio-level impact',       color: C.textSub },
  { key: 'userNotes',            label: 'Your research notes',          color: C.textMuted },
]

// ── Full card (standalone, rendered in the Quant Intelligence page) ────────────

interface CardProps { card: GreeksIntelligenceCard; onSaved?: () => void }

export function GreeksIntelligenceCardView({ card, onSaved }: CardProps) {
  const qc = useQueryClient()
  const [editing, setEditing] = useState(false)
  const [draft, setDraft] = useState<UpdateGreeksIntelligenceRequest>({
    whatItMeasures:       card.whatItMeasures,
    whyItMatters:         card.whyItMatters,
    commonMisuse:         card.commonMisuse,
    positiveEvConditions: card.positiveEvConditions,
    regimeContext:        card.regimeContext,
    sizingImplications:   card.sizingImplications,
    portfolioImpact:      card.portfolioImpact,
    userNotes:            card.userNotes,
  })

  const save = useMutation({
    mutationFn: () => greeksIntelligenceApi.update(card.metricKey, draft),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['greeks-intelligence'] })
      setEditing(false)
      onSaved?.()
    },
  })
        ...
        <div key={key}>
          ...
          {editing ? (
            ...
            <p style={{ margin: 0, fontSize: 13, color: C.textSub, lineHeight: 1.55 }}>
              {card[key as keyof GreeksIntelligenceCard] as string || <em style={{ color: C.textDim }}>—</em>}
            </p>
          )}
        </div>
      ))}
...
// ── Slide-in drawer (opened by GreeksInfoButton) ───────────────────────────────

interface DrawerProps { metricKey: string; onClose: () => void }

export function GreeksIntelligenceDrawer({ metricKey, onClose }: DrawerProps) {
  const { data, isLoading } = useQuery({
    queryKey: ['greeks-intelligence', metricKey],
    queryFn: () => greeksIntelligenceApi.getByKey(metricKey).then(r => r.data.data),
  })
      ...
      <div style={{
        ...
        {!isLoading && !data && (
          <div style={{ color: C.textDim, fontSize: 13 }}>
            No intelligence card found for <strong>{metricKey}</strong>.
          </div>
        )}
        {data && (
          <GreeksIntelligenceCardView card={data} onSaved={onClose} />
        )}
      </div>
    </div>
  )
